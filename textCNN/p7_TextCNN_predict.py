...
tf.app.flags.DEFINE_integer("decay_steps", 6000, "how many steps before decay learning rate.") #批处理的大小 32-->128
...
#tf.app.flags.DEFINE_string("predict_target_file","text_cnn_title_desc_checkpoint/result_padding_1000_classes_5.csv","target file path for final prediction")
tf.app.flags.DEFINE_string("predict_target_file","text_cnn_title_desc_checkpoint/result2.csv","target file path for final prediction")
tf.app.flags.DEFINE_string("predict_source_file",'cnn_test.txt',"target file path for final prediction") #test-zhihu-forpredict-v4only-title.txt
#tf.app.flags.DEFINE_string("word2vec_model_path","fasttext_model.vec","word2vec's vocabulary and vectors") #zhihu-word2vec.bin-100
tf.app.flags.DEFINE_string("word2vec_model_path","model.vec","word2vec's vocabulary and vectors") #zhihu-word2vec.bin-100
...
def get_logits_with_value_by_input(start,end):
    ...
    predicted_labels,value_labels = get_label_using_logits_with_value(logits[0], vocabulary_index2word_label)#这里需要修改，只传入了一个
...
if __name__ == "__main__":
    ...
    #labels,list_value=get_logits_with_value_by_input(0, 20)
    #print("labels:",labels)
