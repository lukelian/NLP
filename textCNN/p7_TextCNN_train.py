...
#train-zhihu4-only-title-all.txt是一个label描述和label_id的文件
#train-zhihu6-title-desc.txt是一个description和label_idn的文件
#tf.app.flags.DEFINE_string("traning_data_path","train-zhihu4-only-title-all.txt","path of traning data.")
#tf.app.flags.DEFINE_string("traning_data_path","train_format_textcnn.txt","path of traning data.")
tf.app.flags.DEFINE_string("traning_data_path","cnn_train.txt","path of traning data.") #O.K.train-zhihu4-only-title-all.txt-->training-data/test-zhihu4-only-title.txt--->'training-data/train-zhihu5-only-title-multilabel.txt'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
#tf.app.flags.DEFINE_string("word2vec_model_path","fasttext_model.vec","word2vec's vocabulary and vectors")
tf.app.flags.DEFINE_string("word2vec_model_path","model.vec","word2vec's vocabulary and vectors") #zhihu-word2vec.bin-100-->zhihu-word2vec-multilabel-minicount15.bin-100
...
def main(_):
    ...
    if 1==1:
        ...
        if FLAGS.multi_label_flag:
            FLAGS.traning_data_path = 'training-data/cnn_train.txt'
            #FLAGS.traning_data_path = 'training-data/train_format_textcnn.txt'
...
def assign_pretrained_word_embedding(sess,vocabulary_index2word,vocab_size,textCNN,word2vec_model_path=None):
    ...
    word2vec_model = models.KeyedVectors.load_word2vec_format(word2vec_model_path, binary=False)
    word2vec_dict = {}
    #for word, vector in zip(word2vec_model.vocab, word2vec_model.vectors):
    for word, vector in zip(word2vec_model.vocab, word2vec_model.syn0):
